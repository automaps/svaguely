...
def read_reqs(file: str, path: Path) -> List[str]:
  ...
  def unroll_nested_reqs(req_str: str, base_path: Path) -> Sequence:
    ...
    if req_str.startswith("-r"):
      with open(base_path / req_str.replace("-r", "").strip()) as file_:
        return [
...
class SvaguelyPackage:
  ...
  def url(self) -> str:
    """description"""
    return "https://github.com/automaps/svaguely"
...
if __name__ == "__main__":
  ...
  setup(
      ...
      python_requires=f">={MIN_PYTHON_MAJOR}.{MIN_PYTHON_MINOR}",
      )
